...
<div class="product">
	<div class="row">
        <?php
            ...
            $sql = "SELECT CK_Name, CK_img, CK_price, CK_ID, Flavor, CK_description, CK_type, CK_Size FROM cake WHERE CK_ID = $ID";
        ...
        ?>
        <div class="proimg">
            <img src="image/<?php echo $row["CK_img"]; ?>" height="550" width="450" />
        </div>    
        <div class="col">
            <div class="category"><p>Home / Cake / <?php echo $row["CK_type"]; ?></p></div>
                <div class="title"><?php echo $row["CK_Name"]; ?></div><br><div class="title2">Flavor: <?php echo $row["Flavor"]; ?></div>
				<div class="price"><p>RM<?php echo $row["CK_price"]; ?></p></div>
				<div class="detail">
                    <h3>Cake Size <i class="fas fa-birthday-cake"></i></h3>
                    <p class="des"><?php echo $row["CK_Size"]; ?> inch</p><br>
                    <h3>Product Details <i class="fa-regular fa-clipboard"></i></h3>
                    <p class="des"><?php echo $row["CK_description"]; ?></p>
                </div>
                <form method="POST" name="a" action="paymentcake.php">
                    <div class="box">
                        <h3>Quantity <i class="fas fa-balance-scale"></i></h3>
                        <input type="hidden" name="cid" value="<?php echo $row["CK_ID"]; ?>">
                        <input type="hidden" name="cimg" value="<?php echo $row["CK_img"]; ?>">
                        <input type="hidden" name="cname" value="<?php echo $row["CK_Name"]; ?>">
                        <input type="hidden" name="csize" value="<?php echo $row["CK_Size"]; ?>">
                        <input type="hidden" name="cprice" value="<?php echo $row["CK_price"]; ?>">
    ...
    <div class="buyh">
                <?php
                    include 'database.php';
                    $sql = "SELECT customer_comment, customer_rating FROM purchase WHERE CK_ID ='".$ID."'";
                    $result = mysqli_query($conn, $sql);
                    $resultCheck = mysqli_num_rows($result);
                ?>
                    <p><b>Comment</b></p>
