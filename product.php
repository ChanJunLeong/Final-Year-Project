    ...
    <div class="home-content">
      ...
      <div class="sales-boxes">
        <div class="recent-sales box">
            <div class="changeo">
                <a href="product.php" class="act">
                    <span class="links_name">Cake</span>
                </a>               
                <a href="productdessert.php" class="act1">
          ...
          <div class="sales-details">
             ...
             <ul class="details">
            <li class="topic">Cake Name</li>
                <?php
                    include 'database.php';
                    $sql = "SELECT * FROM cake WHERE CK_statu = 'Active'";
                    $result7 = mysqli_query($conn, $sql);
                    $result7Check = mysqli_num_rows($result7);
                    while($row = $result7->fetch_assoc())
                    {
                ?>
            <li><?php echo $row["CK_Name"]; ?></li>
                <?php
                    }
                ?>
          </ul>
            <ul class="details">
            <li class="topic">Cake ID</li>
                <?php
                    include 'database.php';
                    $sqla = "SELECT * FROM cake WHERE CK_statu = 'Active'";
                    $result1 = mysqli_query($conn, $sqla);
                    $result1Check = mysqli_num_rows($result1);
                    while($row = $result1->fetch_assoc())
                    {
                ?>
            <li><?php echo $row["CK_ID"]; ?></li>
                <?php
                    }
                ?>
          </ul>
            <ul class="details">
                <li class="topic">Cake Type</li>
                <?php
                    include 'database.php';
                    $sqlb = "SELECT * FROM cake WHERE CK_statu = 'Active'";
                    $result2 = mysqli_query($conn, $sql);
                    $result2Check = mysqli_num_rows($result2);
                    while($row = $result2->fetch_assoc())
                    {
                ?>
            <li><?php echo $row["CK_type"]; ?></li>
                <?php
                    }
                ?>
            </ul>
            <ul class="details">
                <li class="topic">Cake Size</li>
                <?php
                    include 'database.php';
                    $sqlb = "SELECT * FROM cake WHERE CK_statu = 'Active'";
                    $result2 = mysqli_query($conn, $sql);
                    ...
                    {
                ?>
            <li><?php echo $row["CK_Size"]; ?> inch</li>
                <?php
                    }
                ?>
            </ul>
            <ul class="details">
                <li class="topic">Cake Price</li>
                <?php
                    include 'database.php';
                    $sqlb = "SELECT * FROM cake WHERE CK_statu = 'Active'";
                    $result2 = mysqli_query($conn, $sql);
                    ...
                    {
                ?>
            <li>RM<?php echo $row["CK_price"]; ?></li>
                <?php
                    }
                ?>
            </ul>
          ...
          <div class="button">
            <a href="acake.php">Add Cake</a> 
            <a href="ecake.php">Delete Cake</a>
            <a href="editcake.php">Edit Cake</a>
          </div>
        </div>
      </div>
    </div>
