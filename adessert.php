...
<body>
  ...
  <section class="home-section">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <input type="text" name="dflavor" required/><br><br>

      <label for="password">Price:</label>
      <input type="text" name="dprice" required/><br><br>      
      
      <label for="password">Description:</label>
      <input type="text" name="ddes" required/><br><br>           
        
      <label for="password">Image:</label>
      <input type="file" name="dimg" id="password" accept= ".jpg, .jpeg, .png"/><br><br>      
        
      <label for="password">Dessert Type:</label>
      <select name="dtype" id="password" class="tbox" required/>
              <br><br> 
...
<?php
        function button1()
        {
            ...
            if($_FILES["dimg"]["error"]===4)
            {
                echo '<script>alert("Image does not exist")</script>';
            }
            else
            {
                $fileName = $_FILES["dimg"]["name"];
                $fileSize = $_FILES["dimg"]["size"];
                $tmpName = $_FILES["dimg"]["tmp_name"];
                ...
                else
                {
                    ...
                    $sql = "INSERT INTO dessert (Dst_name, Dst_price, Dst_type, Dst_description, Dst_img, Flavor, Dst_statu) VALUES ('".$Dname."', '".$Dprice."', '".$Dtype."', '".$Ddes."', '".$fileName."', '".$Dflavor."', 'Active')";
