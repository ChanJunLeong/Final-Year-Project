...
<div class="product">
	<div class="row">
        <?php
            ...
            $sql = "SELECT Cok_name, Cok_image, Cok_price, Cok_ID, Flavor, Cok_description FROM cookies WHERE Cok_ID = $ID";
        ...
        ?>
        <div class="proimg">
            <img src="image/<?php echo $row["Cok_image"]; ?>" height="550" width="450" />
        </div>    
        <div class="col">
                <div class="title"><?php echo $row["Cok_name"]; ?></div><br><div class="title2">Flavor: <?php echo $row["Flavor"]; ?></div>
				<div class="price"><p>RM<?php echo $row["Cok_price"]; ?></p></div>
				<div class="detail">
                    <h3>Product Details <i class="fa-regular fa-clipboard"></i></h3>
                    <p class="des"><?php echo $row["Cok_description"]; ?></p>
                </div>
                <form method="POST" name="a" action="paymentcookies.php">
                    <div class="box">
                        <h3>Quantity <i class="fas fa-balance-scale"></i></h3>
                        <input type="hidden" name="coid" value="<?php echo $row["Cok_ID"]; ?>">
                        <input type="hidden" name="coimg" value="<?php echo $row["Cok_image"]; ?>">
                        <input type="hidden" name="coname" value="<?php echo $row["Cok_name"]; ?>">
                        <input type="hidden" name="coprice" value="<?php echo $row["Cok_price"]; ?>">
    ...
    <div class="buyh">
                <?php
                    include 'database.php';
                    $sql = "SELECT customer_comment, customer_rating FROM purchase WHERE Cok_ID ='".$ID."'";
                    $result = mysqli_query($conn, $sql);
                    $resultCheck = mysqli_num_rows($result);?>
                    <p><b>Comment</b></p>
                    <?php
